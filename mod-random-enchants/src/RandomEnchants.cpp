...
public:
	...
	{
		uint32 Quality = sObjectMgr.GetItemPrototype(item->GetEntry())->Quality;
		uint32 Class = sObjectMgr.GetItemPrototype(item->GetEntry())->Class;
		...
		if (slotRand[2] != -1)
			chathandle.PSendSysMessage("Newly Acquired |cffFF0000 %s |rhas received|cffFF0000 3 |rrandom enchantments!", sObjectMgr.GetItemPrototype(item->GetEntry())->Name1);
		else if(slotRand[1] != -1)
			chathandle.PSendSysMessage("Newly Acquired |cffFF0000 %s |rhas received|cffFF0000 2 |rrandom enchantments!", sObjectMgr.GetItemPrototype(item->GetEntry())->Name1);
		else if(slotRand[0] != -1)
			chathandle.PSendSysMessage("Newly Acquired |cffFF0000 %s |rhas received|cffFF0000 1 |rrandom enchantment!", sObjectMgr.GetItemPrototype(item->GetEntry())->Name1);
	}
	int getRandEnchantment(Item* item)
	{
		...
		QueryResult* qr = WorldDatabase.PQuery("SELECT enchantID FROM item_enchantment_random_tiers WHERE tier='%d' AND exclusiveSubClass=NULL AND class='%s' OR exclusiveSubClass='%u' OR class='ANY' ORDER BY RAND() LIMIT 1", tier, ClassQueryString.c_str(), sObjectMgr.GetItemPrototype(item->GetEntry())->SubClass);
